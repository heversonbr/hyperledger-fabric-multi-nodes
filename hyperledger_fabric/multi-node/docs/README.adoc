...
One layout configuration file must be used with the terminal emulator _terminator_
and the other wiith _byobu_.
...
Then, copy our configuration file to the `~/.terminator/` directory in your home.
[source, bash]
----
$ cp ./install/terminator_config  ~/.terminator/
----

Finally invoke the terminator layout as follows:
...
As as result  you must have a terminator layout as the one shows below.
